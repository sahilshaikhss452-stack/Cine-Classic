...
export default function ProductionsDesktopCarousel({ productions }: Props) {
  ...
  const goPrev = useCallback(() => setActive((index) => (index - 1 + total) % total), [total]);
  const goNext = useCallback(() => setActive((index) => (index + 1) % total), [total]);
  ...
  return (
    <div style={{ position: 'relative', width: '100%', userSelect: 'none' }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <div
        style={{
          position: 'relative',
            ...
            >
              <div
                style={{
                  ...
                  overflow: 'hidden',
                  position: 'relative',
                  border: isCenter ? '1px solid rgba(212,175,55,0.14)' : '1px solid rgba(255,255,255,0.06)',
                  boxShadow: isCenter
              ...
              >
                ...
                {prod.posterImage && <Image src={prod.posterImage} alt={`${prod.title} poster`} fill sizes="360px" style={{ objectFit: 'cover' }} />}

                {!prod.posterImage && (
                  <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '4.5rem', opacity: 0.18, pointerEvents: 'none' }}>
                ...
                >
                  <div style={{ fontFamily: 'var(--font-playfair), serif', fontSize: '1.1rem', fontWeight: 700, color: 'var(--white)', marginBottom: '0.45rem', lineHeight: 1.25 }}>
                    {prod.title}
                  </div>
      ...
      {total > 1 && (
        <>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '7px', marginTop: '1.8rem' }}>
            {productions.map((_, index) => (
              <button
                ...
                style={{
                  ...
                  height: '7px',
                  borderRadius: '100px',
                  background: index === active ? 'var(--gold)' : 'rgba(255,255,255,0.22)',
                  border: 'none',
                  padding: 0,
                  cursor: 'pointer',
                  flexShrink: 0,
